...
#[cfg(feature = "quic")]
pub fn extract_private_key_from_bytes(
    data: &[u8],
...
/// A helper for managing the CA certificate for authentication.
/// Each time the server is started, a new CA is generated.
/// The CA is used to sign both client and server certificates.
...
/// We'll generate a new client cert and return
/// both the public CA certificate information and that key to the client.
/// The client will use both of those things to connect to the server.
/// Server certificates are generated on-demand as needed by various listeners.
pub struct Pki {
    ca_cert: Certificate,
...
impl Pki {
    ...
    /// Only the CA is created during init; server certificates are generated on-demand
    pub fn init_with_lifetime(cert_lifetime_days: u32) -> anyhow::Result<Self> {
        ...
        alt_names.sort();
        alt_names.dedup();
        log::debug!("generating CA cert with alt_names={alt_names:?}");
        ...
        let mut ca_params = CertificateParams::new(alt_names);
    ...
    /// Generate a server certificate with configurable lifetime and SANs, signed by the CA
    /// This allows generating server certs on-demand with different parameters
    pub fn generate_server_cert(
        ...
        alt_names.sort();
        alt_names.dedup();
        log::debug!("generating server cert with alt_names={alt_names:?}");

        let unix_name = config::username_from_env()?;

        // Calculate certificate validity period
        let now = OffsetDateTime::now_utc();
        let lifetime = Duration::days(lifetime_days as i64);
        ...
        let mut server_pem = server_cert.serialize_pem_with_signer(&self.ca_cert)?;
